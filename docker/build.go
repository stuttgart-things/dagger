...
func (m *ImageBuild) Push(
	...
	// Whether to allow insecure registry connections
	// +optional
	// +default=false
	insecure bool,
) (string, error) {
	...
	// Configure insecure registry (if enabled)
	if insecure {
		// Set the DOCKER_OPTS environment variable to allow insecure registries
		m.Container = m.Container.WithEnvVariable("DOCKER_TLS_VERIFY", "false")
	}
